...
function Test-Image {
    <#
.SYNOPSIS
    Used by Veeam.Documentor to validate supported logo image extension.
.DESCRIPTION
.NOTES
    Version:        0.1.0
...
function Test-Logo {
    <#
    .SYNOPSIS
        Used by Veeam.Documentor to validate logo path.
    .DESCRIPTION
    .NOTES
        Version:        0.1.0
    ...
    PROCESS {
        if ([string]::IsNullOrEmpty($LogoPath)) {
            ...
            if (Test-Image -Path $LogoPath) {
                ...
                throw "New-VeeamDocExport : Logo isn't a supported image file. Please use the following format [.jpeg, .jpg, .png]"
            }
        }
    }
}